...
const setupLogger = () => {
  ...
  const logDirectory = path.resolve(__dirname, "./Results/nal-indexing");

  if (!fs.existsSync(logDirectory)) {
    fs.mkdirSync(logDirectory, { recursive: true });
  }

  const fileName = path.join(logDirectory, `nal-indexing-${timestamp}.txt`);
...
test.describe("NAL-Indexing Automation", () => {
  ...
  test("NAL-Indexing", async ({ page }) => {
    ...
    let keywordsInput = page.locator(
    ...
    const testCases = [
      ...
      { date: "Last 30 Days", keyword: "is OR the OR a" },
    ];
    ...
    for (const scenario of testCases) {
      ...
      await searchBtn.click();
      let textWithKeyword = await getTabText(
        page,
        tabIndex++,
        logToFile,
        false,
