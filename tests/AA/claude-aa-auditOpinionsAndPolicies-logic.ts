...
// import { closeAllOpenTabs, getTabText, parseCount } from "../utils/helpers";

// const IDENTIFIER = "aa_auditOpinionsAndPolicies";

// // How many documents to open & verify per form type.
// const DOCS_TO_PROCESS_PER_FORM = 3;
...
//   const targetRowId = String(targetIndex - 1);
//   const MAX_STAGNANT_SCROLLS = 10;
//   let stagnantScrolls = 0;
//   let lastSeenId = "";

//   while (stagnantScrolls <= MAX_STAGNANT_SCROLLS) {
//     const targetRow = scroller.locator(
//       `div[data-test="resultRow"][id="${targetRowId}"]`,
//     );
...
//     const lastId = (await lastRow.getAttribute("id")) ?? "";
//     stagnantScrolls = lastId === lastSeenId ? stagnantScrolls + 1 : 0;
//     lastSeenId = lastId;
...
//   logToFile(
//     `⚠️ Could not scroll to row ${targetIndex} (id="${targetRowId}") after ${MAX_STAGNANT_SCROLLS} stagnant attempts.`,
//   );
//   return null;
// };
...
//     if (docsToProcess === 0) {
//       rowFindings.push("ℹ️ No results to verify for this form.");
//     }

//     for (let rowIndex = 1; rowIndex <= docsToProcess; rowIndex++) {
//       await switchToLatestResultsTab(page, currentTabIndex);

//       const row = await scrollToRow(page, rowIndex, logToFile);
//       if (!row) {
//         rowFindings.push(`❌ Row ${rowIndex} -> could not scroll into view.`);
//         formValid = false;
//         continue;
//       }
...
//         rowLabel = `Row ${rowIndex} (id=${idAttr})`;
...
//         const rowValid = checks.every((c) => c.isValid);
//         rowFindings.push(
//           `${rowValid ? "✅" : "❌"} ${rowLabel} ->\n${checks
//             .map((c) => c.note)
//             .join("\n")}`,
//         );
//         if (!rowValid) formValid = false;
//       } catch (e: any) {
//         const customMsg = e.message.split("\n")[0];
//         rowFindings.push(`❌ ${rowLabel} -> ${customMsg}`);
//         logToFile(`❌ ${rowLabel} -> ${customMsg}`);
//         formValid = false;
//       }
//     }
...
//       [
//         `Form: ${formValue}`,
//         `Total Docs Found: ${totalDocs}`,
//         `Docs Verified: ${docsToProcess}`,
//         ``,
//         rowFindings.join("\n\n"),
//         ``,
//         `Form Result: ${formValid ? "Valid ✅" : "Invalid ❌"}`,
//       ].join("\n"),
//     );
  ...
  for (const formValue of FORMS_TO_TEST) {
    ...
    for (let rowIndex = 1; rowIndex <= docsToProcess; rowIndex++) {
      ...
      try {
        ...
        const rowValid = checks.every((c) => c.isValid);
        const details = checks.map((c) => c.note);
