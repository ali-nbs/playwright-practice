...
export class SECFilingsPage {
    ...
    async configureDisplayColumns() {
        ...
        const filingInfoPopupCheckbox = this.page.locator('.styles__popupContainer___36f60')
        ...
        const filingInfoCheckbox = this.page.locator('.PopupBody__popup__body___1J_d3')
            .locator('div')
            .filter({ hasText: /^Filing Info$/ })
            .locator('._checkbox__icon_1xotg_257');
        ...
        const accessionCheckbox = this.page.locator('.PopupBody__popup__body___1J_d3')
            .locator('div')
        ...
        const companyInfoPopupCheckbox = this.page.locator('.styles__popupContainer___36f60')
            .filter({ hasText: 'Company Info' })
            .locator('._checkbox__icon_1xotg_257');

        await companyInfoPopupCheckbox.click();

        const companyInfoCheckbox = this.page.locator('.PopupBody__popup__body___1J_d3')
            .locator('div')
            .filter({ hasText: /^Company Info$/ })
            .locator('._checkbox__icon_1xotg_257');
    ...
    async scrapeResults(targetCount: number, formType: string) {
        ...
        while (resultsFound < targetCount) {
            ...
            try {
                ...
                const formTypeCell = currentRow.locator('span').filter({ hasText: new RegExp(`^${formType}`, 'i') }).last();
                ...
                const accessionCell = currentRow.locator('div[data-test="resultCell"]').filter({ hasText: /^\d{10}-\d{2}-\d{6}$/ }).first();
                const accessionNumber = (await accessionCell.count() > 0) ? await accessionCell.innerText() : `Unknown_Row_${resultsFound}`;
                ...
                if (!match || match[1].trim().length === 0) {
                    console.log(`❌ Validation Failed for Row ${resultsFound}: Empty parentheses or no description.`);
                ...
                console.log(`Note: Row ${resultsFound} could not be fully validated. ${e.message}`);
            }
